...
#This script creates a multiple base line plots of variables that is periodically plotted against time
...
#periodically plot the selected data for the given date-times
wkdays <- weekdays(selected_data$Date) #know the weekdays
table(weekdays(selected_data$Date)) #count Friday - 1440, Thursday - 1440
#format "Time" as "POSIXct" "POSIXt"
...
#plot 3
voltage<- as.numeric(as.character(selected_data$Voltage))
#plot4
glob_reactive<- as.numeric(as.character(selected_data$Global_reactive_power))
...
par(mfcol = c(2,2))
with(selected_data,{
        plot(Time, glob_active, type="l",xlab = " ", ylab = "Global Active Power") 
        ...
        legend('topright',bty = "n", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col = c("black", "red", "blue"), lty = 1, lwd = 1)
        plot(Time, voltage, type = "l", xlab="datetime", ylab = "Voltage")
        
        plot(Time, glob_reactive,  type="l",xlab = "datetime",ylab = "Global_reactive_power")
